...
func (r *GPUQuotaReconciler) Reconcile(ctx context.Context, req ctrl.Request) (ctrl.Result, error) {
	...
	llmkubemetrics.ReconcileDuration.WithLabelValues(gpuQuotaControllerName).Observe(time.Since(reconcileStart).Seconds())
	...
	case gq.Spec.Selector != nil:
		...
		if err := r.List(ctx, nsList, client.MatchingLabelsSelector{Selector: labels.SelectorFromSet(gq.Spec.Selector.MatchLabels)}); err != nil {
...
func gpuQuotaCoversNamespace(r client.Client, gq *inferencev1alpha1.GPUQuota, nsName string) bool {
	...
	case gq.Spec.Selector != nil:
		...
		return labels.SelectorFromSet(gq.Spec.Selector.MatchLabels).Matches(labels.Set(ns.Labels))
	}
	return false
}
