...
// #1311: status.UsedGPUCount must reflect the worst-case footprint the HPA
// can reach (autoscaling.maxReplicas), not the authored spec.replicas, so
// the reported usage is truthful while the HPA runs.
func TestReconcileAutoscalingMaxReplicas(t *testing.T) {
	...
	// gpu: 1, replicas: 1, maxReplicas: 100 => 100 GPUs of headroom.
	isvc := &inferencev1alpha1.InferenceService{
		ObjectMeta: metav1.ObjectMeta{
			Name:      "my-isvc",
			Namespace: "my-ns",
	...
	if updatedGQ.Status.UsedGPUCount != 100 {
		t.Errorf("Reconcile() status.UsedGPUCount = %d, want 100 (1 GPU * maxReplicas 100)", updatedGQ.Status.UsedGPUCount)
	}
}
