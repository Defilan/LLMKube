...
//
// A cached file with no marker keeps serving as-is; the marker is
// established when the file is first downloaded under OnChange, mirroring
// the controller's fingerprint-baseline behavior (#635). This avoids an
// unconditional full re-fetch of a multi-GB file on every pod start.
const remoteRevalidateScript = `ETAG_MARKER="$(dirname "$MODEL_PATH")/.$(basename "$MODEL_PATH").etag"; ` +
	`if [ -f "$MODEL_PATH" ] && [ ! -f "$ETAG_MARKER" ]; then ` +
	`echo 'Model cached; revalidation deferred until an etag baseline exists'; exit 0; ` +
	`fi; ` +
