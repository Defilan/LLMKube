...
func TestComputeCacheKey(t *testing.T) {
	...
	for _, tt := range tests {
		t.Run(tt.name, func(t *testing.T) {
			key := controller.ComputeCacheKey(tt.source)

			// Must be exactly 16 hex characters
			if len(key) != 16 {
				t.Errorf("controller.ComputeCacheKey(%q) length = %d, want 16", tt.source, len(key))
			}

			// Must contain only hex characters
			for _, c := range key {
				if (c < '0' || c > '9') && (c < 'a' || c > 'f') {
					t.Errorf("controller.ComputeCacheKey(%q) contains non-hex char %q", tt.source, string(c))
				}
			}
		})
	}
...
func TestComputeCacheKeyDeterministic(t *testing.T) {
	...
	key1 := controller.ComputeCacheKey(source)
...
func TestComputeCacheKeyMatchesController(t *testing.T) {
	...
	expected := controller.ComputeCacheKey(source)
	if key != expected {
...
func TestComputeCacheKeyDifferentSourceDifferentKey(t *testing.T) {
	...
	key1 := controller.ComputeCacheKey("https://huggingface.co/model-a.gguf")
	key2 := controller.ComputeCacheKey("https://huggingface.co/model-b.gguf")
	if key1 == key2 {
