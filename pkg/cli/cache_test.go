...
func TestComputeCacheKeyDeterministic(t *testing.T) {
	...
	if key1 != key2 {
		t.Errorf("cachekey.Compute is not deterministic: %q != %q", key1, key2)
	}
}
...
func TestComputeCacheKeyMatchesController(t *testing.T) {
	...
	if key != expected {
		t.Errorf("cachekey.Compute result changed between calls: %q != %q", key, expected)
	}
}
...
func TestComputeCacheKeyForDeploySources(t *testing.T) {
	// Verify cachekey.Compute works with all source types used by deploy
	tests := []struct {
		name   string
		source string
	}{
