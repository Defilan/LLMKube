...
func TestTokenize_Basic(t *testing.T) {
	cases := []struct {
		...
		{"numbers", "fix bug 42", []string{"bug", "42"}},
		{"empty", "", nil},
		{"only-punct", "!!!", nil},
	}
	for _, tc := range cases {
...
func TestSemanticSimilarity_FunctionWordFabricationScoresLow(t *testing.T) {
	// A fabricated ask sharing only function words with an unrelated body must
	// NOT clear the 0.60 verification threshold after stopword removal.
	claim := "update the version in the file to the new value"
	body := "The reconciler sets Status.Phase to Ready once the deployment is available."
	if got := semanticSimilarity(claim, body); got >= 0.60 {
		t.Fatalf("function-word-only fabrication must score below 0.60, got %.3f", got)
	}
}

func TestSemanticSimilarity_ContentWordMatchVerifies(t *testing.T) {
	// A claim that restates the body's distinctive content words verifies.
	claim := "bind inference servers to an ipv6 host on ipv6-only clusters"
	body := "Inference servers must bind to an ipv6 host on ipv6-only clusters."
	if got := semanticSimilarity(claim, body); got < 0.60 {
		t.Fatalf("content-word match must score >= 0.60, got %.3f", got)
	}
}
