...
func TestApplyDeletedReferenceRailForTask_DiffAgainstActualBase(t *testing.T) {
	...
	//
	// CRITICAL: We use `git clone` here (not `git init` + `git remote add` +
	// `git fetch`) so that `origin/main` exists as a tracking ref. This means
	// `main` resolves to `origin/main` (commit B) in the coder's workspace.
	// Without the production fix, `git diff main...HEAD` succeeds and spans
	// the extra history between commit B and the coder's HEAD, including the
	// removal of the line citing #999. With the fix, the base is resolved to
	// the upstream tip via SHA and the diff is correct.
	ws := filepath.Join(root, "ws")
	if out, err := exec.Command("git", "clone", bare, ws).CombinedOutput(); err != nil {
	...
	// to commit B via origin/main, so the diff spans the coder's commit plus
	// the extra history (commit B removes the line citing #999), and the rail
	// records #999. After the fix, the diff is computed against the upstream
	// tip resolved via SHA and the rail records nothing because the coder's
	// change removes no line citing a ref.
	applyDeletedReferenceRailForTask(context.Background(), task, ws, loopRes,
		func(string) string { return bare })
