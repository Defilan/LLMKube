...
# Go's default 10m test timeout does not fit this suite: it runs ~550s
# against a 600s default, so any stall panics the whole binary. The explicit
# budget makes a stall fail a spec instead.
.PHONY: test-e2e
