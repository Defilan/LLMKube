...
The `GPUQuotaStatus.admissionDenials` counter tracks cumulative denials, and
`lastDenial` records the timestamp of the most recent denial. These are
observable via `kubectl get gpuquota` or the Grafana dashboard.
...
The `llmkube-quota.json` Grafana dashboard (in `docs/grafana/`) visualizes:

- **GPU utilization per quota** — `usedGPUCount / gpuCount` as a gauge.
- **Denial rate** — `admissionDenials` counter over time, broken down by
  quota name.
- **Last denial timestamp** — a stat panel showing when the most recent denial
  occurred.

Import the dashboard from `docs/grafana/llmkube-quota.json` into your Grafana
instance. It reads from the same Prometheus datasource used by the other
LLMKube dashboards.
