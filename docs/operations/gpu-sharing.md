...
When this is empty (the default), shared mode is rejected at admission
with an actionable error. This is intentional: shared workloads collide
with exclusive ones on the same extended resource name, so they must be
steered to a label-separated pool.
...
After applying this, label each MIG-capable node with the desired
partition layout, for example:
...
metadata:
  name: gpu-node-1
  labels:
    nvidia.com/mig.config: 1g.24gb
```

The device plugin then advertises `nvidia.com/mig-1g.24gb` as an
extended resource. Available profiles depend on the GPU model; consult
the NVIDIA MIG User Guide for the full list.

### Time-slicing (shared mode)

To use shared mode on NVIDIA GPUs, configure time-slicing in the NVIDIA
device plugin. This is done via a ConfigMap that the device plugin reads.
Each entry under `data.<key>` contains a `timeSlicing.resources` list
with `name` specifying the resource to time-slice.
...
Exclusive-mode pods must NOT carry this label, or they will collide with
shared-mode pods on the same resource. The operator enforces this at
admission: shared mode is rejected when no pool is configured.
