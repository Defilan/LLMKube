...
`llamacpp:request_duration_seconds` is a histogram of request latency
exposed by the llama.cpp server when `--metrics` is enabled. Scale to
keep p95 latency under a target:
...
spec:
  ...
  metrics:
    - type: Pods
      pods:
        metric:
          name: llamacpp:request_duration_seconds
        target:
          type: AverageValue
  ...
  triggers:
    - type: prometheus
      metadata:
        ...
        query: |
          sum(rate(llamacpp:requests_processing{namespace="default", pod=~"llama-3-8b.*"}[2m]))
          /
