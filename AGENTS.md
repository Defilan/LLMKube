...
- Run `make lint-all` before pushing a PR to catch `//go:build`-tagged files
  that the host's default `GOOS` would silently skip. About 2× slower than
  `make lint`; intentional opt-in so the inner loop stays fast.
