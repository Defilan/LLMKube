...
type InferenceServiceSpec struct {
	...
	// They are added alongside the operator's own flags, but the operator may
	// still append arguments of its own afterward (for example llama.cpp's
	// --metrics), so do not rely on extraArgs overriding a flag by position.
